...
final class Analytics: ObservableObject {
    ...
    func identifyUser(properties: [String: Any]) {
        // First convert the properties to Mixpanel compatible types
        let mixpanelProperties = properties.compactMapValues { convertToMixpanelType($0) }
        
        // Identify user with Mixpanel
        if let userId = properties["id"] as? String {
        ...
        // Track identification event with converted properties
        Mixpanel.mainInstance().track(event: "User_Identified", properties: mixpanelProperties)
    }
...
//        let adjustEvent = ADJEvent(eventToken: "YOUR_ADJUST_EVENT_TOKEN")
